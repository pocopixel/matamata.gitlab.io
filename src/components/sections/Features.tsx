      ...
      <div className="container-editorial">
        <div className="max-w-2xl mb-12">
          ...
          <h2 className="mt-3 text-3xl md:text-4xl font-bold text-primary-900 dark:text-primary-100">
            Kemampuan Monitoring yang Terverifikasi
          </h2>
          <p className="mt-4 text-warm-600 dark:text-warm-400">
            Berikut adalah fitur layanan monitoring FlexiSPY yang telah diverifikasi. Ketersediaan setiap fitur bergantung pada berbagai faktor teknis.
          </p>
        </div>
        ...
        <div className="space-y-px bg-warm-200 dark:bg-warm-700 rounded-xl overflow-hidden">
          {features.map((feature) => (
            <div
              key={feature.number}
              className="group bg-warm-50 dark:bg-warm-800 p-6 md:p-8 hover:bg-white dark:hover:bg-warm-800/80 transition-colors"
            >
              <div className="grid grid-cols-1 md:grid-cols-12 gap-4 md:gap-6 items-start">
                <div className="md:col-span-1">
                  <span className="text-2xl md:text-3xl font-serif font-bold text-primary-300 dark:text-primary-700 group-hover:text-secondary-500 dark:group-hover:text-secondary-500 transition-colors">
                    {feature.number}
                  </span>
                </div>
                <div className="md:col-span-4">
                  <h3 className="text-lg font-bold text-warm-800 dark:text-warm-200">{feature.name}</h3>
                </div>
                <div className="md:col-span-4">
                  <p className="text-sm text-warm-600 dark:text-warm-400 leading-relaxed">{feature.description}</p>
                </div>
                <div className="md:col-span-3 space-y-2">
                  <div>
                    <p className="text-xs font-semibold text-warm-500 dark:text-warm-500 uppercase tracking-wider">Persyaratan</p>
                    <p className="text-sm text-warm-600 dark:text-warm-400 mt-1">{feature.requirement}</p>
                  </div>
                  <div>
                    <p className="text-xs font-semibold text-warm-500 dark:text-warm-500 uppercase tracking-wider">Kompatibilitas</p>
                    <p className="text-sm text-warm-600 dark:text-warm-400 mt-1">{feature.compatibility}</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="mt-8 flex items-start gap-2 p-4 rounded-lg bg-accent-50 dark:bg-accent-900/20 border border-accent-100 dark:border-accent-900/40">
          ...
          <p className="text-sm text-warm-600 dark:text-warm-400">
            Ketersediaan fitur dapat berbeda berdasarkan perangkat, sistem operasi, versi aplikasi, paket, dan ketentuan layanan.
          </p>
        </div>
      </div>
    </section>
